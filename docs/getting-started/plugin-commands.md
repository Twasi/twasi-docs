...
You need to have the same constructor as the super class. Otherwise you will ran into an error when you try to register your command. Just leave it as is.
...
Since StructuredPluginCommand extends TwasiPluginCommand all of the functions are the same. The only difference is that the *execute* function is called *handle* in StructuredPluginCommands. You can find the available functions and methods [here](#setting-up-permissions).
